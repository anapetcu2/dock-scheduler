...
import { DAY_COLUMN_MIN_PX } from "./gridMath";
...
export function ScheduleHeader({ days }: { days: string[] }) {
  return (
    <div
      ...
      style={{ gridTemplateColumns: `repeat(${days.length}, minmax(${DAY_COLUMN_MIN_PX}px, 1fr))` }}
    >
      {days.map((day) => (
        <div
          key={day}
          className={`border-l border-surface-700/60 py-2 ${
        ...
        >
          ...
          <div className={`text-sm ${isTodayISO(day) ? "text-brand-300" : "text-slate-300"}`}>
            {formatDayNumber(day)}
          </div>
        </div>
      ))}
